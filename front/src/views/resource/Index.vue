...
import { dataListFuzzyQuery } from "@/api/request";
...
export default defineComponent({
  ...
  setup() {
    ...
    const getSelection = () => {
      ...
      for (let i = 0; i < multipleSelection.value.length; i++) {
        tempdes =
          ...
          i +
          ":" +
          multipleSelection.value[i].address +
          ";" +
          "\n";
        multipleSelection.value[i].tags.forEach((item: any) => {
          if (tagSum.indexOf(item) == -1) tagSum.push(item);
        });
      }
      console.log(tagSum);
    };
    ...
    const search = async () => {
      ...
      const data = await dataListFuzzyQuery({
        size: 10,
